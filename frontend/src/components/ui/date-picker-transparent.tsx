...
//         let locale: Pick<Locale, "options" | "localize" | "formatLong"> = ru;
...
//                                 }}
//                             >
//                                 <SelectTrigger className="w-fit gap-1 border-none p-0 px-1 focus:bg-accent focus:text-accent-foreground">
//                                     <SelectValue />
//                                 </SelectTrigger>
//                                 <SelectContent>
//                                     {MONTHS.map((month) => (
...
>(
    (
        ...
        const handleSelect = (newDay: Date | undefined) => {
            ...
            // Если дата "Сегодня", просто устанавливаем её
            if (newDay.toDateString() === new Date().toDateString()) {
                onChange?.(newDay);
                setMonth(newDay);
                return;
            }

            // Логика для других дат
            if (!value) {
                onChange?.(newDay);
                setMonth(newDay);
                return;
            ...
            <Popover>
                <PopoverTrigger asChild disabled={disabled}>
                    <Button
                        variant="outline"
                        className={cn(
                            "w-full justify-start hover:bg-transparent text-left bg-transparent border-none h-3 focus:border-[#bcbcbc] focus:outline-none focus:ring-0 focus:ring-[#bcbcbc] focus:ring-offset-2 focus-visible:ring-2 focus-visible:ring-[#bcbcbc] focus-visible:ring-offset-3 [data-aria-hidden]:border-[#bcbcbc] !text-black text-[10px] font-medium leading-3 pl-3",
                            !value && "text-muted-foreground",
                            className
                        )}
                        ref={buttonRef}
                    >
                        {value && <div>до&nbsp;</div>} {formattedDate}
                    </Button>
                </PopoverTrigger>
